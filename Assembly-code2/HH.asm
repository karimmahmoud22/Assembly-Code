...
INLINE_CHAT PROC 
 ...
 CMP PLAYER,2
  ...
  ; get cursor
      ...
      ;mov bh,0h
      ;int 10h
  ; set cursor in the first of this line --> dl = 0   
      mov ah,2
      mov dl,10
      mov dh,YS_2
      int 10h
  ...
  ; return the cursor to the start of the line
      ...
      JMP TransmitX       ; if complete without jumping will display (-)
  ...
  CMP VALUE55,0FH
  JNE CONT_MUS
  RET;;; CLEAR THE NOTIFICATION BAR
  CONT_MUS:
  cmp VALUE55, 28       ; check if the other user pressed enter or not --> to increment the YR
  JNE dont_clearX
  ;JMP clear
  

  ;;;;;;;;;;;;;;;;;;; clear the line --> the other user pressed enter
  ; get cursor
      ;mov ah,3h
      ;mov bh,0h
      ;int 10h
...
FIX_TWO      PROC 
    ...
    JE PL2
      ;------>RECIVE NEW TARGET VALUE 
            ...
            RECIVING_DATAQ:
                ...
                ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;; SEND FLAG
                mov dx , 3FDH ; Line Status Register
                AGAINAQ1: In al , dx ;Read Line Status
                test al , 00100000b
                JZ AGAINAQ1 ;Not empty
    ...
    FALSE2:
        ...
        SEND_INITIALQ:
            ...
            ;Check that Transmitter Holding Register is Empty
            mov dx , 3FDH ; Line Status Register
            AGAINAQ: In al , dx ;Read Line Status
            test al , 00100000b
            JZ AGAINAQ ;Not empty
            ;If empty put the VALUE in Transmit data register
            mov dx , 3F8H ; Transmit data register
            mov al,[SI]
            out dx , al
            ...
            PUSH CX
            ;;;;;;;;;;
            ;;;;;;;;;;;;;;;; WANT TO RECIVE THE FLAG THAT THE OPPSITE IS DONE
...
FIX_ONE      PROC 
    ...
    MOV SI,OFFSET TARGET
        ...
        SEND_INITIALQ2:
            ...
            ;Check that Transmitter Holding Register is Empty
            mov dx , 3FDH ; Line Status Register
            AGAINAQ2: In al , dx ;Read Line Status
            test al , 00100000b
            JZ AGAINAQ2 ;Not empty
            ;If empty put the VALUE in Transmit data register
            mov dx , 3F8H ; Transmit data register
            mov al,[SI]
            out dx , al
...
SENDING_TYPE_FLAG  PROC
  ...
  mov dx , 3FDH ; Line Status Register
  CHK_SEND_FLAG: in al , dx
  test al , 1
  JZ CHK_SEND_FLAG ;Not Ready
  ;If Ready read the VALUE in Receive data register
  mov dx , 03F8H
  in al , dx
  
  REF:RET
  SENDING_TYPE_FLAG ENDP
...
RECIEVING_FLAG PROC
  ...
  MOV VAR,5
  CALL SENDING_DATA ;;; AFTER RECIVING THE FLAG THEN DO YOUR TURN AND SEND AND FLAG SAID THAT "HI I REICIVED"
...
RECIEVING_ASCII PROC  FAR
  ...
  RET
  RECIEVING_ASCII ENDP

RECIEVING_BALL_TYPE PROC
  mov dx , 3FDH ; Line Status Register
  CHK7: in al , dx
  test al , 1
  JZ CHK7 ;Not Ready
...
CHECK_COLLISION_1 PROC 
    ...
    ;;; IF THER IS COLLESION THEN ADD POINT AND CLEAR THE BALL
     MOV TRANS_TYPE,4 ;;; 1 DUE TO START SHOOTING GUN
    CALL SENDING_TYPE_FLAG ;;;; SEND THE TYPE OF DATA TO THE RECIVER AND WAIT TO TAKE THE ANSWER
    CMP AL,5
    MOV VAR,1
    CALL  SENDING_DATA
...
GAME_BALL_CONTROL PROC
    ...
    CMP MASTER,1
    JE YES_CALL_RANDOM
    RET ;;IF THER IS NO BALL AND IAM NOT THE MASTER THEN DOSNT PLAY WITH BALL
    YES_CALL_RANDOM:
    CALL RANDGEN ;; IF THER IS NOT BALL THEN CALL THE RANDOM ONLY IF YOU ARE THE MASTER
     CMP DL,5D
    ...
    MOV TRANS_TYPE,1 ;;; 1 DUE TO START SHOOTING GUN
    CALL SENDING_TYPE_FLAG ;;;; SEND THE TYPE OF DATA TO THE RECIVER AND WAIT TO TAKE THE ANSWER
    CMP AL,5
    ;JNE SET_BALL1
    MOV AL,RANDOM_BALL_TYPE
    MOV VAR,AL ;; HERE WE KNOW THAT THE RECIVER IS FINE THEN SEND THE VALUE TO HIM
    CALL SENDING_DATA
...
ANOTHER_PLAYER_CONTROLER PROC
  MOV TRANS_TYPE,0
  CALL RECIEVING_FLAG

  CMP TRANS_TYPE,0
  JNE PLAY_SECOND_PLAYER
  RET
  PLAY_SECOND_PLAYER:
  CMP TRANS_TYPE,4
  JE YSE_IT_IS_COLLESION
  JMP HALABO2A
  YSE_IT_IS_COLLESION:
    CALL RECIEVING_SCAN
    CMP CARRY_ACTION,1
    JNE DEALING_WITH_ADD_2
    CALL ADD_POINT_GAME_1
    CALL   FINISH_BALL_GAME
    RET
    DEALING_WITH_ADD_2:
  ...
  HALABO2A:
  CMP TRANS_TYPE,1
  JNE ISNT_BALL_CONTROL
  CALL RECIEVING_BALL_TYPE ;; IF IT IS 1 THEN WE KNOW IT IS BALL CONTROL THEN RECIVE IT AND GET OUT
  MOV IS_BALL,1
  MOV IS_BALL_INTIAL,0
  ...
  RET 
  ISNT_BALL_CONTROL:
  CMP TRANS_TYPE,2 ;;; IF 2 THEN IT IS SCAN CODE AND DEALING WITH ARROWS - Z - ENTER - F4
  JE YSE_IS_SCAN
  JMP CHARACTER_3
  YSE_IS_SCAN:
  CALL RECIEVING_SCAN
   ...
   CMP PLAYER,2
      ...
      ;  CALL DRAW_TRIANGLE_1 ;AL ;; DRWA THE TRIANGLE IN BLACK 
       ...
       CALL DRAW_TRIANGLE_1 ;AL;; DRAW THE TRIANGLE BUT WITH THE CORRECT COLOR
        ...
        UP_3: CMP CARRY_ACTION,72D  ;;UP  
        JE IS_UP_3 
        JMP DOWN_3
        IS_UP_3:
        CMP shifting_Y_1,100D ;; IF IT IS LEFT PRESS THEN CHECK THE LIMIT THAT WE NEED TO PRINT INSIE IT 
        JA LAS3_3
        JMP END_RECIVE_KEE_PRESS
        LAS3_3:
        MOV TRI_COLOR1,0
       ; CALL DRAW_TRIANGLE_1 ;; DRWA THE TRIANGLE IN BLACK 
        ...
        DOWN_3: 
        CMP CARRY_ACTION,80D ;;DOWN  
        JE IS_DOWN_3 
        JMP Z_3
        IS_DOWN_3:
        CMP shifting_Y_1,215D ;;195 IF IT IS LEFT PRESS THEN CHECK THE LIMIT THAT WE NEED TO PRINT INSIE IT 
        JNA LAS4_3
        JMP END_RECIVE_KEE_PRESS
        LAS4_3:
        MOV AL,0
     ...
     ;  CALL DRAW_TRIANGLE_2 ;; ;; DRWA THE TRIANGLE IN BLACK 
       ...
       ; CALL GAME_bullet_CONTROLE
        ...
        JMP NUM_0_3
        IS_NUM9_3: 
         CMP shifting_Y_2,415D ;; IF IT IS LEFT PRESS THEN CHECK THE LIMIT THAT WE NEED TO PRINT INSIE IT 
        JNA LAS9_3
        JMP END_RECIVE_KEE_PRESS
        LAS9_3:
      ...
      ; CALL DRAW_TRIANGLE_2 ;; ;; DRWA THE TRIANGLE IN BLACK 
       ...
       CALL DRAW_TRIANGLE_2 ;AL;; DRAW THE TRIANGLE BUT WITH THE CORRECT COLOR
        ...
        INLINE_CHAT_3:
        CMP CARRY_ACTION,0FH
        JNE ENTER_PRESS_3
        MOV HE_IS_INLINE_CHAT,1
        RET
       ...
       ; MOV ACTUALSIZE,0
        ...
        BACK_SCAPEE:
        CMP PLAYER,2
        JE PLAYER1_INS_READ_PRINT_3
        JMP PLAYER2_INS_READ_PRINT_3
        PLAYER1_INS_READ_PRINT_3:
         MOV INS_COLOR,0h
        CALL PRINT_PLAYER_COMMAND_1
  ...
  END_RECIVE_KEE_PRESS:
  RET
  ANOTHER_PLAYER_CONTROLER ENDP
...
GAME_CONTROL PROC  FAR
    ...
    MOV CARRY_ASCII,0
     MOV HE_IS_ENTER,0
    MOV HE_IS_INLINE_CHAT,0
    MOV IS_F4,0
        CALL GAME_bullet_CONTROLE_1
        CALL GAME_bullet_CONTROLE_2
        CALL GAME_BALL_CONTROL
        CALL ANOTHER_PLAYER_CONTROLER
        CMP HE_IS_ENTER,1
        JNE CHECK_HE_PAUSE
        RET
        CHECK_HE_PAUSE:
        CMP IS_F4,1
        JNE CHECK_INLINE_CHAT
        RET 
        CHECK_INLINE_CHAT:
        CMP HE_IS_INLINE_CHAT,1
        JNE TAKE_ACTION
      ...
      ;        CALL GAME_bullet_CONTROLE
       ...
       JMP IAM_NUMBER2
        ...
        MOV TRANS_TYPE,2 ;;; 2 DUE TO SENDING SCAN CODE
        CALL SENDING_TYPE_FLAG ;;;; SEND THE TYPE OF DATA TO THE RECIVER AND WAIT TO TAKE THE ANSWER
        CMP AL,5
       ; JNE IS_RIGHT
        MOV AL,CARRY_ACTION
     ...
     ;  CALL DRAW_TRIANGLE_2 ;; ;; DRWA THE TRIANGLE IN BLACK 
      ...
      ;  JNE IS_PAUS
        ...
        CMP CARRY_ACTION,28D
        JE IS_ENTER_PRESS
        JMP INLINE_CHAT_PREESS
        IS_ENTER_PRESS:
        
        CMP MASTER,1
        JE DEALING_WITH_ENTER
        JMP REACT_USER ;; IF IT ISNOT MY TURN THEN DOSNT SERVICE AND CHARACTER FROM ME 
        DEALING_WITH_ENTER:
      ...
      ;  JNE IS_ENTER_PRESS
       ...
       ; MOV DL,MASTER
        ...
        CMP CARRY_ACTION, 0EH ;;;; CHECK FOR THE BACK DELETE
        JNE NOT_BACK_ELETE
        MOV TRANS_TYPE,2
        CALL SENDING_TYPE_FLAG 
        CMP AL,5
        JNE CHARACTER
         MOV AL,CARRY_ACTION
        MOV VAR,AL ;; HERE WE KNOW THAT THE RECIVER IS FINE THEN SEND THE VALUE TO HIM
        CALL SENDING_DATA
        JMP FINISH_SEND_BACK


        NOT_BACK_ELETE:
        MOV TRANS_TYPE,3 ;;; 3 DUE TO SENDING ASCII CODE
        CALL SENDING_TYPE_FLAG ;;;; SEND THE TYPE OF DATA TO THE RECIVER AND WAIT TO TAKE THE ANSWER
        CMP AL,5
        JNE CHARACTER
        MOV AL,CARRY_ASCII
   ...
   CHECK_FORTH:
    ...
    NOTPOW1:  
        ...
        NOT_MINUS9:    
         ...
         CMP MASTER,1
        JNE RECIVE_FORBIDEN4
        MOV AH,0
        INT 16H
        MOV VAR,AL
        CALL SENDING_DATA
        MOV AL,VAR
        JMP END_RECIVE_FORBIDEN3
        RECIVE_FORBIDEN4:
        CALL RECIEVING_ASCII
        MOV AL,CARRY_ASCII

        END_RECIVE_FORBIDEN3:
        CMP AL,30H 
        JE ONME      
            MOV AH,CARRY1
            MOV CARRY,AH
 ...
 ;CALL TAKE_COMMAND_PLAYER_1   
   ...
   CHECK_FORTH1:
    ...
    NOTPOW11:  
        ...
        CMP MASTER,1
        JNE RECIVE_FORBIDEN3
        MOV AH,0
        INT 16H
        MOV VAR,AL
        CALL SENDING_DATA
        MOV AL,VAR
        JMP END_RECIVE_FORBIDEN2
        RECIVE_FORBIDEN3:
        CALL RECIEVING_ASCII
        MOV AL,CARRY_ASCII

        END_RECIVE_FORBIDEN2:
        CMP AL,30H
        JE ONME1O  
            ;ADD DI,37
            MOV AH,CARRY2
