...
EXTRN F1_F2_F3_PAGES:FAR

EXTRN FORBIDDEN_CHAR_S_R:FAR

EXTRN SEND_R_NAMES:FAR
EXTRN SEND_R_CARACTERS:FAR
EXTRN SEND_S_CARACTERS:FAR
...
FOBIDONE DB "SELECT THE FORBIDDEN CHAR  : $"
...
PLAY_GAME:
    ...
    ;CMP AL,POINT1
    ;JB EQUAL_POINT2 
    ;MOV AL,POINT1        ;DETERMINE INTIAL VALUE
    ;MOV POINT2,AL
    ;JMP POINTS
    ...
    ;CALL SEND_REC
     ...
     CALL SEND_R_NAMES
    CALL SEND_R_CARACTERS
    CALL SEND_S_CARACTERS
    
    CMP PAGE_NUMBER,3
    JE CHAT             ; HE WANTS TO CHAT
    CMP PAGE_NUMBER,0
    JE GAME             ; HE WANTS TO GAME
    JMP END_PROGRAM
    
    
    CHAT:
         CALL CHAT_PAGE      ;AFTER END OF CHAT ( WE MUST RETURN THE MESSAGE ON THE NOTIFICATION BAR )
         CMP PAGE_NUMBER,2
         JE SECOND_PAGE         ; HE WANTS TO GAME
   
    GAME:
         ;COMPARE BETWWEN TWO INTIAL POINTS 
    ...
    E:
        ...
        SEND_INITIAL:
            ...
            AGAINA: In al , dx ;Read Line Status
            test al , 00100000b
            JZ AGAINA ;Not empty
    ...
    E2:
        ...
        SEND_INITIAL1:
            ...
            ;Check that Transmitter Holding Register is Empty
            mov dx , 3FDH ; Line Status Register
            AGAIN0: In al , dx ;Read Line Status
            test al , 00100000b
            JZ AGAIN0 ;Not empty
            ;If empty put the VALUE in Transmit data register
            mov dx , 3F8H ; Transmit data register
 ...
 ;  INT 21H     
  ...
  ;  CMP LEVEL,31H
        ...
        SEND1:  ;;;;;;; IF PLAYER ONE WRITE THE INS AND WANT TO EXCUTE IN PLAYER 2
        call PLAYER_ONE_INS
        CALL CLEAR_INS
        CALL LOSS_ONE
        CMP LOSSER_ONE,31H
     ...
     TWO_WINNER:
               ...
               CMP PLAYER,1
               JNE O1
               MOV DX,OFFSET TWO_WIN                ;CHANGE TO YOU LOSS
               MOV BH,0
               MOV AH,9
               INT 21H
               JMP LAB1
               O1:
               MOV DX,OFFSET ONE_WIN                ;CHANGE TO YOU LOSS
               MOV BH,0
               MOV AH,9
               INT 21H
               JMP LAB1
